...
#include "engine/ecs/ecs.h" /* Para ri_ecs_load_world */
#include "engine/ui/layout.h"
#include "engine/ui/ui.h"
#include "game/simulation/components/sim_components.h" /* [NEW] Metadata Type */
...
#define COLOR_BG ((struct ri_ui_color){ 0.006f, 0.006f, 0.008f, 1.0f })

/* Primary Button: Vibrant Royal Blue */
/* sRGB ref: #3b5bdb -> Linear: ~0.045, 0.105, 0.7 */
#define COLOR_PRIMARY ((struct ri_ui_color){ 0.045f, 0.105f, 0.700f, 1.0f })
#define COLOR_PRIMARY_HOVER                                                    \
	((struct ri_ui_color){ 0.080f, 0.180f, 0.900f, 1.0f })

/* Secondary Button: Dark Surface */
/* sRGB ref: #2d2d35 -> Linear: ~0.027, 0.027, 0.038 */
#define COLOR_SECONDARY ((struct ri_ui_color){ 0.027f, 0.027f, 0.035f, 1.0f })
#define COLOR_SECONDARY_HOVER                                                  \
	((struct ri_ui_color){ 0.045f, 0.045f, 0.055f, 1.0f })

/* Text */
#define COLOR_TEXT                                                             \
	((struct ri_ui_color){ 0.70f, 0.72f, 0.80f,                           \
				1.0f }) /* Grey-ish white */
#define COLOR_TEXT_DIM                                                         \
	((struct ri_ui_color){ 0.15f, 0.16f, 0.22f, 1.0f }) /* Dark Grey */
#define COLOR_BORDER ((struct ri_ui_color){ 0.04f, 0.04f, 0.06f, 1.0f })
...
static bool custom_button(ri_ui_ctx_t ctx, const char *label,
			  const char *icon_char, struct ri_ui_rect rect,
			  bool is_primary)
{
	int32_t mx, my;
	...
	if (!is_primary) {
		ri_ui_draw_rect_outline(ctx, rect, COLOR_BORDER, 1.0f);
	}
	...
	if (label) {
		...
		float text_w = ri_ui_measure_text(ctx, label, font);
		float centerX = rect.x + (rect.width / 2.0f);

		if (icon_w > 0)
			centerX += 10.0f;

		ri_ui_draw_text(ctx, label, centerX - (text_w / 2.0f),
		...
		while ((dir = readdir(d)) != NULL) {
			...
			if (strstr(dir->d_name, ".bin") &&
				...
				/* ID of Metadata Component? We need the TYPE ID.
                   ...
                   We need the ID used by `ri_ecs_save_world`. 
                   ...
                   Looking at `ecs.h`: `typedef uint32_t ri_component_type`.
                   The IDs are assigned by the user.
                   We need to know the ID for RI_COMP_METADATA.
                   ...
                   Let's assume there is an enum `ri_component_type_id` or similar.
                ...
                */
				/* [Assuming implicit ID for now, but peek needs explicit ID]
                   ...
                   I will add a TODO and assume RI_COMP_METADATA is the macro name.
                */
				...
				if (ri_ecs_peek_metadata(item->full_path,
							  &meta, sizeof(meta),
							  RI_COMP_METADATA)) {
			   ...
			   int win_h)
{
	...
	float title_w = ri_ui_measure_text(ctx, title, title_size);
	ri_ui_draw_text(ctx, title, cx - (title_w / 2.0f), header_y + 60.0f,
	...
	struct ri_ui_rect rect_main = { cx - (container_w / 2.0f), btn_start_y,
					 container_w, btn_h };
	...
	struct ri_ui_rect rect_solar = { cx - (container_w / 2.0f), sub_y,
					  sub_w, btn_h };
	...
	struct ri_ui_rect rect_tls = { cx + (gap / 2.0f), sub_y, sub_w,
					btn_h };
	...
	if (g_workspace_count > 0) {
		/* Label "Workspaces" */
		ri_ui_draw_text(ctx, "Workspaces", cx - (container_w / 2.0f),
	...
	if (g_workspace_count == 0) {
		...
		float empty_w = ri_ui_measure_text(ctx, empty_msg, 12.0f);
		ri_ui_draw_text(ctx, empty_msg, cx - (empty_w / 2.0f),
		...
		for (int i = 0; i < g_workspace_count; i++) {
			...
			struct ri_ui_rect rect = { cx - (container_w / 2.0f),
						    curr_y, container_w,
						    list_item_h };
			...
			if (g_renaming_index == i) {
				...
				if (ri_ui_key_pressed(ctx, RI_KEY_ENTER)) {
					...
					g_renaming_index = -1;
				} else if (ri_ui_key_pressed(ctx,
							      RI_KEY_ESCAPE)) {
					/* CANCEL */
					g_renaming_index = -1;
				} else {
					ri_ui_text_field(ctx, rect_edit,
							  g_rename_buf, 63,
							  &g_focus_rename);
				}
				...
				ri_ui_draw_text(ctx, info_buf, rect.x + pad_x,
						 rect.y + 30, 11.0f,
						 COLOR_TEXT_DIM);
				...
				ri_ui_draw_rect(ctx, rect_edit,
						 h_edit ? COLOR_SECONDARY_HOVER
							: COLOR_BG);
				ri_ui_draw_rect_outline(ctx, rect_edit,
							 COLOR_BORDER, 1.0f);
				ri_ui_draw_text(ctx, "E", rect_edit.x + 8,
						 rect_edit.y + 5, 12.0f,
						 COLOR_TEXT); /* 'E' icon */

				if (h_edit && ri_ui_mouse_clicked(ctx, 0)) {
					g_renaming_index = i;
				...
				ri_ui_draw_rect(
					ctx, rect_del,
					h_del ? (struct ri_ui_color){ 0.3f,
								       0.1f,
								       0.1f,
								       1.0f }
					      : COLOR_BG); /* Red-ish on hover */
				ri_ui_draw_rect_outline(ctx, rect_del,
							 COLOR_BORDER, 1.0f);
				ri_ui_draw_text(ctx, "X", rect_del.x + 8,
						 rect_del.y + 5, 12.0f,
						 COLOR_TEXT); /* 'X' icon */

				if (h_del && ri_ui_mouse_clicked(ctx, 0)) {
					/* DELETE ACTION */
	...
	float more_w = ri_ui_measure_text(ctx, more, 12.0f);
	ri_ui_draw_text(ctx, more, cx - (more_w / 2.0f), curr_y + 15.0f, 12.0f,
			 COLOR_TEXT_DIM);
}
