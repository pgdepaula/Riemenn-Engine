#ifndef RI_UI_RENDER_VISUAL_UTILS_H
#define RI_UI_RENDER_VISUAL_UTILS_H
...
static inline int ri_visual_find_parent(int my_index,
...
static inline void ri_visual_transform_point(
	double px, double py, double pz, /* Input Real Pos */
	double body_radius,		 /* Object Radius */
	enum ri_body_type type,	 /* Object Type */
	const struct ri_body *bodies, int n_bodies, ri_visual_mode_t mode,
	...
	float *out_x, float *out_y, float *out_z, float *out_radius)
{
	...
	/* 3. Determine Radius */
	float mult = (type == RI_BODY_STAR) ? rad_mult_star : rad_mult_planet;
	...
	/* Find Parent */
	int parent_idx =
		ri_visual_find_parent(my_index, bodies, n_bodies, NULL);
	...
	if (parent_idx == -1) {
		...
		*out_z = (float)pz;
		...
		/* Parent Params */
		const struct ri_body *parent = &bodies[parent_idx];
		...
		bool parent_is_root =
			(ri_visual_find_parent(parent_idx, bodies, n_bodies,
						NULL) == -1);
		...
		if (parent_is_root) {
			...
			float p_mult = (parent->type == RI_BODY_STAR)
					       ? rad_mult_star
					       : rad_mult_planet;
	...
	if (use_gravity_well) {
		...
		*out_y = 0.0f;
		...
		int curr = my_index;
		while (curr != -1) {
			int p = ri_visual_find_parent(curr, bodies, n_bodies,
						       NULL);
			if (p == -1)
				break;
			curr = p;
...
static inline void
ri_visual_calculate_transform(const struct ri_body *target,
			       const struct ri_body *bodies, int n_bodies,
			       ri_visual_mode_t mode, float *out_x,
			       float *out_y, float *out_z, float *out_radius)
{
	...
	ri_visual_transform_point(target->state.pos.x, target->state.pos.y,
...
#endif /* RI_UI_RENDER_VISUAL_UTILS_H */
