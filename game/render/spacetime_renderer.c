...
#include <stdio.h>
#include <string.h>
#include "game/simulation/data/orbit_marker.h"
#include "game/simulation/components/sim_components.h" /* [FIX] For Visual Flags */
#include "game/render/planet_renderer.h" /* For 3D Lines */
...
static void calculate_sphere_uv(const ri_camera_t *cam, float width,
				 ...
				 struct ri_planet_pass *planet_pass)
{
	const ri_view_assets_t *assets =
		(const ri_view_assets_t *)assets_void;
	...
	if (tex_bh) {
		struct ri_ui_color white = { 1.0f, 1.0f, 1.0f, 1.0f };
		ri_ui_draw_quad_uv(ctx, tex_bh, 0, 0, 0, 0, (float)width, 0, 1,
	...
	if (tex_bg) {
		...
		struct ri_ui_color space_color = { 1.0f, 1.0f, 1.0f, 1.0f };
		for (int y = 0; y < segs_y; y++) {
			for (int x = 0; x < segs_x; x++) {
				...
				ri_ui_draw_quad_uv(ctx, tex_bg, x0, y0, u0, v0,
	...
	/* 2. Bodies */
	int n_bodies = 0;
	const struct ri_body *bodies = ri_scene_get_bodies(scene, &n_bodies);
	...
	if (assets && n_bodies > 0 && !fixed_mode) {
		...
		struct ri_ui_color red = { 1.0f, 0.2f, 0.2f, 0.7f };
		...
		for (int i = 0; i < n_bodies; i++) {
			...
			if (bodies[i].visual_flags & RI_VISUAL_FLAG_SHOW_VECTORS) {
				show_vector = true;
			}
			...
			for (int j = 0; j < n_bodies; j++) {
				if (i == j) continue;
				if (bodies[j].type == RI_BODY_ASTEROID) continue; /* Ignore debris */
			...
			if (best_attractor != -1) {
				...
				float px, py, pz, pr;
				ri_visual_calculate_transform(&bodies[i], bodies,
							       n_bodies, mode, &px, &py,
							       &pz, &pr);

				float ax, ay, az, ar;
				ri_visual_calculate_transform(&bodies[best_attractor], bodies,
							       n_bodies, mode, &ax, &ay,
							       &az, &ar);

				float spx, spy;
				ri_project_point(cam, px, py, pz, (float)width,
						  (float)height, &spx, &spy);
				
				float sax, say;
				ri_project_point(cam, ax, ay, az, (float)width,
				...
				if ((spx > -100 && spx < width + 100 && spy > -100 &&
				    ...
				    (sax > -100 && sax < width + 100 && say > -100 &&
				     say < height + 100)) {
					ri_ui_draw_line(ctx, spx, spy, sax, say,
							 red, 2.0f);
				}
			}
		}
	...
	if (assets && n_bodies > 0) {
		...
		struct ri_ui_color purple = { 0.8f, 0.4f, 1.0f, 0.8f };

		int attractor_idx = -1;
		double max_mass = -1.0;
		for (int i = 0; i < n_bodies; i++) {
			if (bodies[i].type == RI_BODY_STAR ||
			    bodies[i].type == RI_BODY_BLACKHOLE) {
		...
		for (int i = 0; i < n_bodies; i++) {
			...
			if (bodies[i].visual_flags & RI_VISUAL_FLAG_SHOW_TRAIL) {
				show_green = true;
			}
			...
			int parent_idx = ri_visual_find_parent(i, bodies,
								n_bodies, NULL);
			...
			if (parent_idx != -1 && parent_idx != attractor_idx) {
				...
				ri_visual_calculate_transform(
					&bodies[i], bodies, n_bodies, mode, &px,
					&py, &pz, &pr);

				float tx, ty, tz, tr;
				ri_visual_calculate_transform(
					&bodies[parent_idx], bodies, n_bodies,
					mode, &tx, &ty, &tz, &tr);
				...
				if (show_purple) {
					if (planet_pass) {
						...
						ri_project_point(cam, px, py,
								  pz,
								  (float)width,
								  (float)height,
								  &sx1, &sy1);
						ri_project_point(cam, tx, ty,
								  tz,
								  (float)width,
								  (float)height,
								  &sx2, &sy2);
						ri_ui_draw_line(ctx, sx1, sy1,
								 sx2, sy2,
								 purple, 1.5f);
					}
				}
				...
				if (show_green) {
					struct ri_ui_color green_trail = {
						0.2f, 1.0f, 0.2f, 0.6f
					};
					/* ... Trail Logic ... */
					...
					const struct ri_body *parent =
						&bodies[parent_idx];
					int history_len = bodies[i].trail_count;
					if (parent->trail_count < history_len)
						history_len =
							parent->trail_count;

					float rad_mult_parent =
						(mode == RI_VISUAL_MODE_DIDACTIC ||
						 mode == RI_VISUAL_MODE_CINEMATIC)
							? 1200.0f
							: 1.0f;
					if (parent->type == RI_BODY_STAR)
						rad_mult_parent = 100.0f;
					float parent_vis_radius =
						(float)parent->state.radius *
						rad_mult_parent;
					float rad_mult_moon =
						(mode == RI_VISUAL_MODE_DIDACTIC ||
						 mode == RI_VISUAL_MODE_CINEMATIC)
							? 1200.0f
							: 1.0f;
					float moon_vis_radius =
						(float)bodies[i].state.radius *
						rad_mult_moon;
					...
					for (int k = 1; k < history_len; k++) {
                        ...
                        */
						...
						/* Parent Vis 1 */
						float p_vis_x1, p_vis_y1,
							p_vis_z1, p_vis_r1;
						ri_visual_transform_point(
							parent->trail_positions
								[p_idx_curr][0],
							parent->trail_positions
								[p_idx_curr][1],
							parent->trail_positions
						...
						double mz1 =
							bodies[i].trail_positions
								[idx_curr][2];
						double dist1 = sqrt(
							pow(mx1 - parent->trail_positions
									    [p_idx_curr]
									    [0],
							    2) +
							pow(my1 - parent->trail_positions
									    [p_idx_curr]
									    [1],
							    2) +
							pow(mz1 - parent->trail_positions
									    [p_idx_curr]
									    [2],
							    2));
						if (dist1 < 1.0)
						...
						double mz2 =
							bodies[i].trail_positions
								[idx_prev][2];
						double dist2 = sqrt(
							pow(mx2 - parent->trail_positions
									    [p_idx_prev]
									    [0],
							    2) +
							pow(my2 - parent->trail_positions
									    [p_idx_prev]
									    [1],
							    2) +
							pow(mz2 - parent->trail_positions
									    [p_idx_prev]
									    [2],
							    2));
						if (dist2 < 1.0)
	...
	if (assets && n_bodies > 0) {
		struct ri_ui_color trail_color = { 0.2f, 0.5f, 1.0f, 0.6f };

		for (int i = 0; i < n_bodies; i++) {
			const struct ri_body *planet = &bodies[i];
			if (planet->type != RI_BODY_PLANET)
				continue;
			if (assets->isolated_body_index >= 0 &&
			    i != assets->isolated_body_index)
				continue;
			...
			if (planet->visual_flags & RI_VISUAL_FLAG_SHOW_TRAIL) {
				show = true;
			}
			if (!show) continue;

			int dynamic_parent = ri_visual_find_parent(
				i, bodies, n_bodies, NULL);
			if (dynamic_parent != -1 &&
			    bodies[dynamic_parent].type == RI_BODY_PLANET)
				continue; /* Skip moons here */

			if (planet->trail_count < 2)
				continue;

			for (int j = 0; j < planet->trail_count - 1; j++) {
				int start_idx = (planet->trail_head -
						 planet->trail_count + j +
						 RI_MAX_TRAIL_POINTS) %
						RI_MAX_TRAIL_POINTS;
				int end_idx =
					(start_idx + 1) % RI_MAX_TRAIL_POINTS;

				float x1 =
					planet->trail_positions[start_idx][0];
				float y1 =
					planet->trail_positions[start_idx][1];
				float z1 =
					planet->trail_positions[start_idx][2];
				...
				ri_visual_transform_point(
					x1, y1, z1, planet->state.radius,
					planet->type, bodies, n_bodies, mode, i,
					&tx1, &ty1, &tz1, &tr1);

				ri_visual_transform_point(
					x2, y2, z2, planet->state.radius,
					planet->type, bodies, n_bodies, mode, i,
					&tx2, &ty2, &tz2, &tr2);

				float sx1, sy1, sx2, sy2;
				ri_project_point(cam, tx1, ty1, tz1,
						  (float)width, (float)height,
						  &sx1, &sy1);
				ri_project_point(cam, tx2, ty2, tz2,
						  (float)width, (float)height,
						  &sx2, &sy2);

				bool on_screen =
					(sx1 > -100 && sx1 < width + 100 &&
					 sy1 > -100 && sy1 < height + 100);
				if (on_screen) {
					ri_ui_draw_line(ctx, sx1, sy1, sx2,
							 sy2, trail_color,
							 1.5f);
				}
			}
	...
	if (assets && assets->orbit_markers &&
        ...
        */
		struct ri_ui_color purple = { 0.6f, 0.2f, 0.8f, 1.0f };
		struct ri_ui_color purple_outline = { 0.9f, 0.6f, 1.0f, 0.9f };
		struct ri_ui_color green = { 0.2f, 1.0f, 0.2f, 1.0f };
		struct ri_ui_color green_outline = { 0.6f, 1.0f, 0.6f, 0.9f };

		const struct ri_orbit_marker_system *sys =
			assets->orbit_markers;
		...
		for (int i = 0; i < sys->marker_count; i++) {
			const struct ri_orbit_marker *m = &sys->markers[i];
			if (!m->active)
				continue;
		...
		for (int i = 0; i < sys->marker_count; i++) {
			...
			if (m->parent_index >= 0 &&
			    m->parent_index < n_bodies) {
				if (bodies[m->parent_index].type ==
					    RI_BODY_PLANET ||
				    bodies[m->parent_index].type ==
					    RI_BODY_MOON) {
					is_moon_orbit = true;
				}
			}
			...
			if (is_moon_orbit) {
				...
				if (m->planet_index >= 0 && m->planet_index < n_bodies) {
					if (bodies[m->planet_index].visual_flags & RI_VISUAL_FLAG_SHOW_MARKERS) {
						show = true;
					}
				}
				if (!show) continue;
			...
			if (m->planet_index >= 0 &&
			    m->planet_index < n_bodies) {
				radius_for_calc =
					bodies[m->planet_index].state.radius;
			}

			float tx, ty, tz, tr;
			ri_visual_transform_point(
				m_x, 0.0f, m_z, radius_for_calc,
				RI_BODY_PLANET, bodies, n_bodies, mode,
				m->planet_index, &tx, &ty, &tz, &tr);

			float sx, sy;
			ri_project_point(cam, tx, ty, tz, (float)width,
					  (float)height, &sx, &sy);

			if (sx < -50 || sx > width + 50 || sy < -50 ||
			    sy > height + 50)
				continue;

			float size = 10.0f;
			struct ri_ui_color dot_color =
				is_moon_orbit ? green : purple;
			struct ri_ui_color outline_color =
				is_moon_orbit ? green_outline : purple_outline;
			struct ri_ui_color ring_color = dot_color;
			ring_color.a = 0.3f;

			ri_ui_draw_circle_fill(ctx, sx, sy, size, dot_color);
			ri_ui_draw_circle_fill(ctx, sx, sy, size + 2.0f,
						ring_color);

			char orbit_text[16];
			snprintf(orbit_text, sizeof(orbit_text), "#%d",
				 m->orbit_number);
			float orbit_tw =
				ri_ui_measure_text(ctx, orbit_text, 12.0f);
			ri_ui_draw_text(ctx, orbit_text, sx - orbit_tw * 0.5f,
					 sy + size + 2.0f, 12.0f,
					 outline_color);
		}
	}
	...
	for (int i = 0; i < n_bodies; i++) {
		...
		if (assets && assets->isolated_body_index >= 0) {
			if (i != assets->isolated_body_index &&
				...
				int parent = ri_visual_find_parent(
					i, bodies, n_bodies, NULL);
		...
		ri_visual_calculate_transform(b, bodies, n_bodies, mode,
					       &visual_x, &visual_y, &visual_z,
					       &visual_radius);

		float sx, sy;
		ri_project_point(cam, visual_x, visual_y, visual_z, width,
				  height, &sx, &sy);
		...
		struct ri_ui_color color = { (float)b->color.x,
					      (float)b->color.y,
		...
		if (b->type == RI_BODY_PLANET || b->type == RI_BODY_STAR ||
			...
			if (sx > 0 && sx < width && sy > 0 && sy < height) {
				...
				float tw = ri_ui_measure_text(ctx, label,
							       font_size);
				ri_ui_draw_text(ctx, label, sx - tw * 0.5f,
						 sy + s_radius + 5.0f,
						 font_size, RI_UI_COLOR_WHITE);
			}
		} else {
			ri_ui_draw_circle_fill(ctx, sx, sy, s_radius, color);
		}
	}
}
