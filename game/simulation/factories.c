...
#include "game/simulation/data/blackhole.h"
					      ...
					      struct ri_vec3 color)
{
	...
	b.state.pos = pos;
	b.state.mass =
		(mass > 0.0) ? mass : 0.01; /* Earth-like relative to BH */
	b.state.radius = radius;
	b.state.acc = (struct ri_vec3){ 0, 0, 0 };
	b.state.vel = (struct ri_vec3){ 0, 0, 0 };
	b.state.rot_axis = (struct ri_vec3){ 0, 1, 0 };
	b.state.rot_speed = 7.27e-5; /* Earth rotation rad/s */
	b.state.moment_inertia =
		0.4 * b.state.mass * radius * radius; /* Sphere */
	b.state.shape = RI_SHAPE_SPHERE;

	/* Type Specifics */
	b.type = RI_BODY_PLANET;
	b.prop.planet =
		(struct ri_planet_data){ .physical_state = RI_STATE_SOLID,
					    ...
					    struct ri_vec3 color)
{
	...
	b.state.moment_inertia =
		0.07 * b.state.mass * radius * radius; /* Central condensed */
	b.state.shape = RI_SHAPE_SPHERE;

	/* Type Specifics */
	b.type = RI_BODY_STAR;
	b.prop.star =
		(struct ri_star_data){ .luminosity = 3.828e26,	  /* Watts */
					.temp_effective = 5772.0, /* Kelvin */
					.age = 4.6e9,		  /* Years */
					.stage = RI_STAR_MAIN_SEQUENCE,
					.metallicity = 0.0122,
					.spectral_type = "G2V" };
						 ...
						 double mass, double radius)
{
	...
	b.prop.bh = (struct ri_blackhole_data){
		.spin_factor = 0.9,
		.event_horizon_r = 2.0 * b.state.mass,
		.ergososphere_r =
			2.0 *
			b.state.mass, /* Simplified (static limit at equator) */
		.accretion_disk_mass = 0.01 * b.state.mass
	};

	/* Visuals */
	b.color = (struct ri_vec3){ 0.0, 0.0, 0.0 };
	return b;
}
					  ...
					  struct ri_vec3 pos)
{
	...
	b.state.rot_axis = (struct ri_vec3){ sin(desc->axis_tilt),
					      cos(desc->axis_tilt), 0.0 };
	...
	default:
		...
		b.prop.planet.rotation_period =
			desc->rotation_period; /* [FIX] Copiar dado */
	...
	b.is_alive = true;
	b.is_fixed = (b.type == RI_BODY_STAR && pos.x == 0 && pos.y == 0);
					      ...
					      struct ri_vec3 pos)
{
	...
	b.state.mass = desc->mass;
	b.state.radius = desc->radius;
	b.state.rot_axis = (struct ri_vec3){ sin(desc->axis_tilt), 0.0,
					      cos(desc->axis_tilt) };
	...
	b.state.moment_inertia =
		0.07 * b.state.mass * desc->radius * desc->radius;
	b.state.shape = RI_SHAPE_SPHERE;
	b.state.acc = (struct ri_vec3){ 0, 0, 0 };
	b.state.vel = (struct ri_vec3){ 0, 0, 0 };
			     ...
			     struct ri_vec3 pos)
{
	...
	b.state.moment_inertia = 0.4 * b.state.mass * desc->event_horizon_r *
				 desc->event_horizon_r;
	b.state.shape = RI_SHAPE_SPHERE;
	b.state.acc = (struct ri_vec3){ 0, 0, 0 };
	b.state.vel = (struct ri_vec3){ 0, 0, 0 };
