...
enum {
	RI_COMP_CELESTIAL = RI_COMP_COUNT,
	RI_COMP_ORBIT_DESC,
	RI_COMP_ORBITAL,
	SIM_COMP_COUNT
};
 ...
 */
#define RI_ORBITAL_FLAG_TIDAL_LOCK (1 << 0)

/**
 * struct ri_orbital_component
 * Define relacionamentos pai-filho e parâmetros orbitais.
 */
typedef struct {
	ri_entity_id parent;	/* ID da Entidade do corpo orbitado (Pai) */
...
typedef enum {
	...
	RI_CELESTIAL_MOON,
	RI_CELESTIAL_STAR,
	RI_CELESTIAL_BLACKHOLE,
	RI_CELESTIAL_ASTEROID
} ri_celestial_type;
...
typedef struct {
	...
	bool has_atmosphere;
	char composition[64];
	struct ri_vec3 color; // Cor Visual
	/* Parâmetros de Rotação */
	struct ri_vec3 rotation_axis;
...
typedef struct {
	...
	struct ri_vec3 color; // Cor Visual da Estrela
} ri_star_component;
 ...
 */
typedef enum {
	RI_VISUAL_FLAG_SHOW_TRAIL = (1 << 0),
	RI_VISUAL_FLAG_SHOW_MARKERS = (1 << 1),
	RI_VISUAL_FLAG_SHOW_VECTORS = (1 << 2), // Gravity/Velocity lines
	RI_VISUAL_FLAG_SHOW_LABEL = (1 << 3)
} ri_visual_flags;

/**
 * struct ri_celestial_component
 * Propriedades de lógica de jogo para corpos celestes.
 */
typedef struct {
	ri_celestial_type type;
	char name[64];
	...
	union {
		ri_planet_component planet;
		ri_star_component star;
	} data;
} ri_celestial_component;

/**
 * struct ri_metadata_component
