 ...
 * F = (3Ṁ / 8π) * (1/r³) * (1/(-E)) * Q(r)
 */
static double novikov_thorne_Q(const struct ri_kerr *bh, double r)
{
	/*
 ...
 */
static double kerr_circular_energy(const struct ri_kerr *bh, double r)
{
	/*
...
double ri_disk_isco(const struct ri_kerr *bh)
{
	...
	return ri_kerr_isco(bh, true); /* Prograde */
}
...
double ri_disk_velocity_phi(const struct ri_kerr *bh, double r)
{
   ...
   */
	double omega_k = ri_disk_omega_kepler(bh, r);
	double omega_frame = ri_kerr_omega_frame(bh, r, M_PI / 2.0);
			       ...
			       double observer_inclination)
{
   ...
   */
	...
	double v_phi = ri_disk_velocity_phi(bh, r);
			       ...
			       double inclination)
{
   ...
   */
	double z = ri_disk_redshift_total(bh, r, phi, inclination);
	return 1.0 / (1.0 + z);
}
					      ...
					      double z)
{
   ...
   */
	struct ri_color_rgb result;

	/* Fator de brilho (beaming) */
	double g = 1.0 / (1.0 + z);
	double brightness =
		g * g * g * g; /* g^4 para intensidade bolométrica */
...
{
	/* Temperatura local */
	double temp = ri_disk_temperature(bh, disk, r);

	if (temp < 0.001) {
		/* Fora do disco */
		return (struct ri_color_rgb){ 0.0f, 0.0f, 0.0f };
	}

	/* Cor base do corpo negro */
	struct ri_color_rgb base_color = ri_blackbody_color(temp);

	/* Redshift total */
	double z = ri_disk_redshift_total(bh, r, phi, inclination);

	/* Aplica redshift */
	return ri_color_apply_redshift(base_color, z);
}
