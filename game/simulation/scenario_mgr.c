...
#include "game/simulation/presets/presets.h"
...
{
	RI_LOG_INFO("Carregando Sistema Solar...");
	ri_preset_solar_system(app->scene);
	return true;
}

static bool load_earth_sun(struct app_state *app)
{
	RI_LOG_INFO("Carregando Sol, Terra e Lua...");
	ri_preset_earth_moon_sun(app->scene);
	return true;
}

static bool load_kerr_blackhole(struct app_state *app)
{
	RI_LOG_INFO("Carregando Black Hole Kerr...");

	struct ri_vec3 center = { 0, 0, 0 };
	struct ri_vec3 zero = { 0, 0, 0 };
	struct ri_vec3 black = { 0, 0, 0 };

	/* Black hole central */
	ri_scene_add_body(app->scene, RI_BODY_BLACKHOLE, center, zero, 10.0,
			   2.0, black);
	...
	for (int i = 0; i < 8; i++) {
		...
		struct ri_vec3 pos = { .x = r * cos(angle),
					.y = 0.0,
					.z = r * sin(angle) };
		struct ri_vec3 vel = { .x = -v * sin(angle),
					.y = 0.0,
					.z = v * cos(angle) };
		struct ri_vec3 col = { .x = 0.3f + 0.1f * i,
					.y = 0.5f,
					.z = 1.0f - 0.1f * i };

		ri_scene_add_body(app->scene, RI_BODY_PLANET, pos, vel, 0.1,
				   0.5, col);
	}
...
{
	RI_LOG_INFO("Carregando Sistema Binário...");
	...
	struct ri_vec3 yellow = { 1.0, 0.9, 0.3 };
	struct ri_vec3 orange = { 1.0, 0.5, 0.2 };

	/* Estrela 1 */
	ri_scene_add_body(app->scene, RI_BODY_STAR,
			   (struct ri_vec3){ -r1, 0, 0 },
			   (struct ri_vec3){ 0, 0, v_orb * r1 / separation },
			   m1, 2.0, yellow);

	/* Estrela 2 */
	ri_scene_add_body(app->scene, RI_BODY_STAR,
			   (struct ri_vec3){ r2, 0, 0 },
			   (struct ri_vec3){ 0, 0, -v_orb * r2 / separation },
			   m2, 1.5, orange);

	/* Planeta orbitando o sistema */
	double r_planet = 50.0;
	double v_planet = sqrt(total_m / r_planet);
	ri_scene_add_body(app->scene, RI_BODY_PLANET,
			   (struct ri_vec3){ r_planet, 0, 0 },
			   (struct ri_vec3){ 0, 0, v_planet }, 0.1, 0.4,
			   (struct ri_vec3){ 0.3, 0.5, 1.0 });
...
{
	RI_LOG_INFO("Carregando cenário de debug (SINGLE PLANET)...");
	...
	ri_scene_add_body(app->scene, RI_BODY_PLANET,
			   (struct ri_vec3){ 0, 0, 0 }, /* Pos 0,0,0 */
			   (struct ri_vec3){ 0, 0, 0 }, /* Static */
			   1.0, 1.0,			 /* Mass, Radius=1.0 */
			   (struct ri_vec3){ 1.0, 0.0, 1.0 }
			   /* Magenta Color */
	);

	/* REFERENCE PLANET (Blue) at (20, 0, 0) */
	ri_scene_add_body(app->scene, RI_BODY_PLANET,
			   (struct ri_vec3){ 30.0, 0, 0 }, /* Pos 30,0,0 */
			   (struct ri_vec3){ 0, 0, 0 },    /* Static */
			   1.0, 1.0, /* Mass, Radius=1.0 */
			   (struct ri_vec3){ 0.0, 1.0, 1.0 } /* Cyan Color */
	);
...
{
	RI_LOG_INFO("Carregando cenário Terra-Lua (Isolado)...");
	ri_preset_earth_moon_only(app->scene);
	return true;
}

static bool load_jupiter_pluto_pull(struct app_state *app)
{
	RI_LOG_INFO("Carregando cenário Júpiter & Plutão Pull...");
	ri_preset_jupiter_pluto_pull(app->scene);
	return true;
}
...
{
	if (!app || !app->scene) {
		RI_LOG_ERROR("scenario_load: app ou scene nulos");
		return false;
	}
	...
	RI_LOG_INFO("scenario_load: Loading Checkpoint A (Type=%d)", type);
	...
	case SCENARIO_DEBUG:
		...
		break;
	default:
		RI_LOG_WARN("Cenário desconhecido: %d", type);
		return false;
	}

	RI_LOG_INFO("scenario_load: Checkpoint B (OK=%d)", ok);
	...
	if (ok) {
		...
		RI_LOG_INFO("scenario_load: Time reset to 0.0");

		RI_LOG_INFO("Cenário '%s' carregado com sucesso",
			     scenario_get_name(type));
	} else {
		RI_LOG_ERROR("scenario_load: Failed to load scenario type %d",
			      type);
	}
...
void scenario_unload(struct app_state *app)
{
	...
	/*
	 * TODO: Implementar ri_scene_clear() na engine
	 * Por enquanto, removemos corpo por corpo (ineficiente mas funciona)
	 */
	int count = 0;
	ri_scene_get_bodies(app->scene, &count);
	RI_LOG_INFO("scenario_unload: Cleaning up %d bodies...", count);

	int safety = 0;
	while (count > 0 && safety++ < 1000) {
		ri_scene_remove_body(app->scene, 0);	  // Always remove head
		ri_scene_get_bodies(app->scene, &count); // Update count
	}

	if (count > 0) {
		RI_LOG_ERROR("scenario_unload: Failed to remove all bodies. "
			      "Remaining: %d",
			      count);
	}

	/* Reseta contadores de nomes para próximo cenário começar do 1 */
	ri_scene_reset_counters();
...
bool scenario_save_snapshot(struct app_state *app)
{
	...
	ri_entity_id meta_id = ri_ecs_create_entity(world);

	ri_metadata_component meta = { .accumulated_time =
	...
	ri_ecs_add_component(world, meta_id, RI_COMP_METADATA, sizeof(meta),
			      &meta);
	...
	/* 3. Save */
	RI_LOG_INFO("Salvando Snapshot: %s ('%s')", filename, display_name);
	bool ok = ri_ecs_save_world(world, filename);

	/* 4. Cleanup Metadata (don't want it persisting in runtime memory) */
	ri_ecs_destroy_entity(world, meta_id);
...
bool scenario_load_from_file(struct app_state *app, const char *filename)
{
	...
	RI_LOG_INFO("Carregando Workspace: %s", filename);
	...
	/* Load Binary */
	if (!ri_ecs_load_world(world, filename)) {
		RI_LOG_ERROR("Falha ao carregar arquivo binário.");
		return false;
	}

	/* Search for Metadata & Restore State */
	ri_ecs_query q;
	ri_ecs_query_init(&q, world, (1 << RI_COMP_METADATA));

	ri_entity_id meta_id;
	if (ri_ecs_query_next(&q, &meta_id)) {
		ri_metadata_component *meta = ri_ecs_get_component(
			world, meta_id, RI_COMP_METADATA);
		if (meta) {
		...
		RI_LOG_WARN("Metadados não encontrados no save (Legacy?). "
			     "Resetando tempo.");
...
bool scenario_reload_current(struct app_state *app)
{
	if (!app || app->current_workspace[0] == '\0') {
		RI_LOG_WARN("Nenhum workspace ativo para recarregar.");
		return false;
	}
