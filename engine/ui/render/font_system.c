...
{
	if (!ctx || ctx->font.initialized)
		return RI_UI_OK;

	RI_LOG_INFO("Inicializando Sistema de Fontes Riemann...");

	/* 1. Localiza a fonte */
	char *font_path = find_system_font("sans-serif");
	if (!font_path) {
		RI_LOG_ERROR("Falha ao encontrar fonte do sistema!");
		return RI_UI_ERR_INIT;
	}
	RI_LOG_INFO("  > Usando fonte: %s", font_path);

	/* 2. Inicializa FreeType */
	FT_Library ft;
	if (FT_Init_FreeType(&ft)) {
		RI_LOG_ERROR("Falha ao inicializar FreeType!");
		free(font_path);
		return RI_UI_ERR_INIT;
	}
	...
	if (FT_New_Face(ft, font_path, 0, &face)) {
		RI_LOG_ERROR("Falha ao carregar face da fonte!");
		FT_Done_FreeType(ft);
		free(font_path);
		return RI_UI_ERR_INIT;
	}
	...
	for (int c = 32; c < 256; c++) {
			...
			for (uint32_t x = 0; x < g->bitmap.width; x++) {
				uint8_t alpha =
					g->bitmap.buffer[y * g->bitmap.width +
							 x];
				size_t idx = ((y_offset + y) * atlas_dim +
					      (x_offset + x)) *
					     4;
		...
		/* Salva info do glyph */
		struct ri_glyph_info *info = &ctx->font.glyphs[c];
	...
	if (ri_gpu_texture_create(ctx->device, &tex_cfg,
				   &ctx->font.atlas_tex) != RI_GPU_OK) {
		RI_LOG_ERROR("Falha ao criar textura de atlas de fonte!");
	} else {
		ri_gpu_texture_upload(ctx->font.atlas_tex, 0, 0, atlas_data,
				       atlas_dim * atlas_dim * 4);
	}
	...
	RI_LOG_INFO("Sistema de Fontes pronto. Visual premium ativado.");
...
void ri_font_system_shutdown(struct ri_ui_ctx_impl *ctx)
{
	...
	if (ctx->font.atlas_tex) {
		ri_gpu_texture_destroy(ctx->font.atlas_tex);
	}
