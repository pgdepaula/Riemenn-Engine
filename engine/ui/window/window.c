...
static void ui_event_callback(ri_window_t window,
			      const struct ri_event *event, void *userdata)
{
	(void)window;
	struct ri_ui_ctx_impl *ctx = userdata;
	if (!ctx)
		return;
	...
	case RI_EVENT_KEY_REPEAT:
		if (event->key.scancode < RI_UI_MAX_KEYS) {
			ctx->input.keys[event->key.scancode] = true;
		}
		break;

	case RI_EVENT_KEY_UP:
		if (event->key.scancode < RI_UI_MAX_KEYS) {
				...
				const struct ri_ui_config *config)
{
	...
	/* === Inicializa Platform === */
	ret = ri_platform_init(&ctx->platform);
	...
	/* === Cria Janela === */
	struct ri_window_config win_config = {
		...
		.flags = config->resizable ? RI_WINDOW_RESIZABLE : 0,
	};

	ret = ri_window_create(ctx->platform, &win_config, &ctx->window);
	if (ret != RI_PLATFORM_OK) {
		fprintf(stderr,
			"[ui/window] erro: falha ao criar janela (%d)\n", ret);
		ri_platform_shutdown(ctx->platform);
		ctx->platform = NULL;
		return RI_UI_ERR_WINDOW;
	}
	...
	/* Registra callback */
	ri_window_set_event_callback(ctx->window, ui_event_callback, ctx);
...
void ri_ui_window_poll_events(ri_ui_ctx_t ctx)
{
	if (!ctx || !ctx->platform)
		return;
	ri_platform_poll_events(ctx->platform);
}
