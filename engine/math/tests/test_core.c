/**
 * @file test_core.c
 * @brief Driver de teste CLI para libri_core
 *
 * "Confia, mas verifica."
	...
	do {                                                                   \
		...
		if (diff > (eps)) {                                            \
			fprintf(stderr, "%s %s: exp %f, got %f (eps %e)\n",    \
				TEST_FAIL, msg, (double)(b), (double)(a),      \
				(double)(eps));                                \
			tests_failed++;                                        \
		} else {                                                       \
			/* printf("%s %s\n", TEST_PASS, msg); */               \
...
void test_vec4_math()
{
	struct ri_vec4 a = ri_vec4_make(1, 2, 3, 4);
	struct ri_vec4 b = ri_vec4_make(10, 20, 30, 40);
	...
	double dot = ri_vec4_dot_minkowski(a, a);
...
void test_metric_invert()
{
	...
	struct ri_metric m = ri_metric_minkowski();
	struct ri_metric inv;

	int ret = ri_metric_invert(&m, &inv);
	...
	struct ri_metric diag = ri_metric_diag(-2.0, 0.5, 4.0, 1.0);
	ret = ri_metric_invert(&diag, &inv);
...
{
	struct ri_schwarzschild bh = { .M = 1.0 }; /* rs = 2.0 */
	struct ri_metric g;

	/* r = 4.0, theta = PI/2 */
	ri_schwarzschild_metric(&bh, 4.0, 1.57079632679, &g);
   ...
   * sin^2(90) = 16 */
	...
	double z = ri_schwarzschild_redshift(&bh, 4.0);
...
int main()
{
	...
	if (tests_failed == 0) {
		printf("\n\033[32mTODO CORE OPERANDO EM NÍVEL "
		       "KERNEL.\033[0m\n");
		return 0;
	} else {
		printf("\n\033[31mFALHAS DETECTADAS. AUDITE O "
		       "CÓDIGO!\033[0m\n");
		return 1;
	}
}
