...
#ifndef RI_CORE_MATH_VEC4_H
#define RI_CORE_MATH_VEC4_H
...
/**
 * struct ri_vec4 - Vetor 4D em espaço-tempo
...
/**
 * struct ri_vec3 - Vetor 3D espacial (pra facilitar)
 *
...
/**
 * ri_vec4_make - Cria vetor 4D
 */
static inline struct ri_vec4 ri_vec4_make(real_t t, real_t x, real_t y,
					    real_t z)
{
	return (struct ri_vec4){ .t = t, .x = x, .y = y, .z = z };
}

/**
 * ri_vec4_zero - Vetor nulo
 */
static inline struct ri_vec4 ri_vec4_zero(void)
{
	return (struct ri_vec4){ 0 };
}

/**
 * ri_vec3_make - Cria vetor 3D
 */
static inline struct ri_vec3 ri_vec3_make(real_t x, real_t y, real_t z)
{
	return (struct ri_vec3){ .x = x, .y = y, .z = z };
}

/**
 * ri_vec3_zero - Vetor 3D nulo
 */
static inline struct ri_vec3 ri_vec3_zero(void)
{
	return (struct ri_vec3){ 0 };
}
...
/**
 * ri_vec4_dot_minkowski - Produto interno com métrica de Minkowski
 *
 * η_μν = diag(-1, +1, +1, +1)
 *
...
/**
 * ri_vec4_norm2_minkowski - Norma ao quadrado (Minkowski)
 *
...
/**
 * ri_vec4_is_null - Verifica se é vetor nulo (fóton)
 *
...
/**
 * ri_vec4_is_timelike - Verifica se é timelike
 */
bool ri_vec4_is_timelike(struct ri_vec4 v);

/**
 * ri_vec4_is_spacelike - Verifica se é spacelike
 */
...
/**
 * ri_vec4_spatial - Extrai parte espacial de vec4
 */
static inline struct ri_vec3 ri_vec4_spatial(struct ri_vec4 v)
{
	return (struct ri_vec3){ .x = v.x, .y = v.y, .z = v.z };
}

/**
 * ri_vec3_to_vec4 - Promove vec3 para vec4 com t dado
 */
static inline struct ri_vec4 ri_vec3_to_vec4(struct ri_vec3 v, real_t t)
{
	return (struct ri_vec4){ .t = t, .x = v.x, .y = v.y, .z = v.z };
}
...
/**
 * ri_vec3_to_spherical - Cartesianas para esféricas
...
/**
 * ri_vec3_from_spherical - Esféricas para cartesianas
 */
struct ri_vec3 ri_vec3_from_spherical(real_t r, real_t theta, real_t phi);

#endif /* RI_CORE_MATH_VEC4_H */
