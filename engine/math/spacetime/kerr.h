...
#ifndef RI_CORE_SPACETIME_KERR_H
#define RI_CORE_SPACETIME_KERR_H
...
/**
 * struct ri_kerr - Parâmetros do buraco negro de Kerr
...
/**
 * ri_kerr_Sigma - Função Σ(r, θ)
 *
 * Σ = r² + a² cos²θ
 *
 * Aparece em toda parte na métrica de Kerr.
 */
static inline double ri_kerr_Sigma(const struct ri_kerr *bh, double r,
				    double theta)
{
...
/**
 * ri_kerr_Delta - Função Δ(r)
 *
 * Δ = r² - 2Mr + a²
 *
 * Zeros de Δ: horizontes de eventos (externo e interno).
 */
static inline double ri_kerr_Delta(const struct ri_kerr *bh, double r)
{
...
/**
 * ri_kerr_horizon_outer - Horizonte de eventos externo r+
 *
 * r+ = M + √(M² - a²)
 *
 ...
 */
double ri_kerr_horizon_outer(const struct ri_kerr *bh);

/**
 * ri_kerr_horizon_inner - Horizonte de Cauchy interno r-
 *
 * r- = M - √(M² - a²)
 *
 ...
 */
double ri_kerr_horizon_inner(const struct ri_kerr *bh);

/**
 * ri_kerr_ergosphere - Raio da ergoesfera
 *
 ...
 */
double ri_kerr_ergosphere(const struct ri_kerr *bh, double theta);

/**
 * ri_kerr_isco - ISCO (Innermost Stable Circular Orbit)
 ...
 */
double ri_kerr_isco(const struct ri_kerr *bh, bool prograde);
...
/**
 * ri_kerr_omega_frame - Velocidade angular do frame dragging
 *
 ...
 */
double ri_kerr_omega_frame(const struct ri_kerr *bh, double r, double theta);
...
/**
 * ri_kerr_metric - Calcula tensor métrico de Kerr
 ...
 */
void ri_kerr_metric(const struct ri_kerr *bh, double r, double theta,
		     struct ri_metric *out);

/**
 * ri_kerr_metric_inverse - Calcula métrica inversa g^μν
 */
void ri_kerr_metric_inverse(const struct ri_kerr *bh, double r, double theta,
			     struct ri_metric *out);
...
/**
 * ri_kerr_redshift_zamo - Redshift para observador ZAMO
 * @bh: parâmetros
 * @r: coordenada radial
 * @theta: ângulo polar
 *
 ...
 */
double ri_kerr_redshift_zamo(const struct ri_kerr *bh, double r,
			      double theta);
...
/**
 * ri_kerr_metric_func - Wrapper para ri_christoffel_compute
 *
 * Use como ri_metric_func com userdata = struct ri_kerr*
 *
 * Coordenadas em vec4: (t, r, θ, φ)
 */
void ri_kerr_metric_func(struct ri_vec4 coords, void *userdata,
			  struct ri_metric *out);

#endif /* RI_CORE_SPACETIME_KERR_H */
