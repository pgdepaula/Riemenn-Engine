...
#define RI_G_SI 6.67430e-11

/** Velocidade da luz (SI: m/s) */
#define RI_C_SI 299792458.0

/** Unidade Astronômica (SI: m) */
#define RI_AU_SI 1.495978707e11

/** Massa Solar (SI: kg) */
#define RI_MASS_SUN_SI 1.98847e30

/** Raio Solar (SI: m) */
#define RI_RADIUS_SUN_SI 6.9634e8
 ...
 */
#define RI_SCALE_DISTANCE (50.0 / RI_AU_SI)

/**
 * ESCALA DE MASSA
 * M☉ (1.989e30 kg) → 20 unidades
 */
#define RI_SCALE_MASS (20.0 / RI_MASS_SUN_SI)
 ...
 */
#define RI_SCALE_RADIUS (3.0 / RI_RADIUS_SUN_SI)
...
#define RI_SIM_MASS_SUN 20.0

/** Raio do Sol em unidades de simulação */
#define RI_SIM_RADIUS_SUN 3.0

/** 1 AU em unidades de simulação */
#define RI_SIM_AU 50.0
...
#define RI_RATIO_MASS_JUPITER_SUN (1.0 / 1047.348)

/** Razão massa Terra / Sol */
#define RI_RATIO_MASS_EARTH_SUN (1.0 / 332946.0)

/** Razão raio Júpiter / Sol */
#define RI_RATIO_RADIUS_JUPITER_SUN 0.10045

/** Razão raio Terra / Sol */
#define RI_RATIO_RADIUS_EARTH_SUN 0.00916
...
#define RI_METERS_TO_SIM(m) ((m) * RI_SCALE_DISTANCE)

/** Converte kg para unidades de simulação (massa) */
#define RI_KG_TO_SIM(kg) ((kg) * RI_SCALE_MASS)

/** Converte metros para unidades de simulação (raio) */
#define RI_RADIUS_TO_SIM(m) ((m) * RI_SCALE_RADIUS)

/** Converte AU para unidades de simulação */
#define RI_AU_TO_SIM(au) ((au) * RI_SIM_AU)
...
static inline double ri_orbital_velocity(double central_mass_sim,
					  double radius_sim)
{
	if (radius_sim <= 0.0)
		return 0.0;
 ...
 *   T = 2π√(r³/M)
 */
static inline double ri_orbital_period(double central_mass_sim,
					double radius_sim)
{
...
/** Segundos por dia */
#define RI_SECONDS_PER_DAY 86400.0

/** Segundos por ano (não-bissexto) */
#define RI_SECONDS_PER_YEAR (365.0 * RI_SECONDS_PER_DAY)

/** 
 * Epoch J2000.0 como Unix timestamp
 * 2000-01-01 12:00:00 UTC = 946728000 Unix time
 */
#define RI_J2000_UNIX 946728000

/**
 * ri_sim_time_to_date - Converte segundos desde J2000.0 para data legível
 ...
 */
static inline void ri_sim_time_to_date(double sim_seconds, int *year,
					int *month, int *day, int *hour,
					int *minute, int *second)
{
	/* Dias desde J2000.0 */
	double total_days = sim_seconds / RI_SECONDS_PER_DAY;
	...
	double day_seconds = frac_day * RI_SECONDS_PER_DAY;
	/* J2000 começa ao meio-dia (12:00), então somamos 12 horas */
	day_seconds += 12.0 * 3600.0;
	if (day_seconds >= RI_SECONDS_PER_DAY) {
		day_seconds -= RI_SECONDS_PER_DAY;
		total_days += 1.0;
	}
