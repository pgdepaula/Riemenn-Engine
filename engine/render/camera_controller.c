				  ...
				  double dt)
{
	...
	/* Shift Boost */
	if (ri_ui_key_down(ctx, RI_KEY_LEFTSHIFT)) {
		move_speed *= 5.0f;
	}
	...
	/* Mouse Pan (Left Button + Drag) */
	{
		...
		if (ri_ui_mouse_down(ctx, 0)) { /* Left Button */
			int32_t mx, my;
	...
	if (ri_ui_key_down(ctx, RI_KEY_Q))
		cam->y += move_speed;
	if (ri_ui_key_down(ctx, RI_KEY_E))
		cam->y -= move_speed;
}
