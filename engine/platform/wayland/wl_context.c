			    ...
			    uint32_t version)
{
	...
	if (strcmp(interface, wl_compositor_interface.name) == 0) {
		...
		p->xdg_wm_base = wl_registry_bind(registry, name,
...
int ri_platform_init(ri_platform_t *platform)
{
	...
	struct ri_platform_impl *p = calloc(1, sizeof(*p));
	if (!p)
	...
	if (!p->xkb_ctx) {
		RI_WL_LOG("erro: xkb_context_new falhou");
		free(p);
		return RI_PLATFORM_ERR_INIT;
	}

	/* Connect Display */
	p->display = wl_display_connect(NULL);
	if (!p->display) {
		RI_WL_LOG("erro: wl_display_connect falhou");
	...
	if (!p->compositor || !p->xdg_wm_base) {
		RI_WL_LOG("erro: compositor faltando interfaces "
			   "(compositor/xdg_wm_base)");
		ri_platform_shutdown(p);
		return RI_PLATFORM_ERR_INIT;
	}
...
void ri_platform_poll_events(ri_platform_t platform)
{
	...
	if (wl_display_dispatch(p->display) == -1) {
		RI_WL_LOG("erro: wl_display_dispatch falhou");
	}
}
