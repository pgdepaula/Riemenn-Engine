			  ...
			  wl_fixed_t sx, wl_fixed_t sy)
{
	...
	struct ri_platform_impl *p = data;
			   ...
			   uint32_t state)
{
	...
	switch (button) {
	case 0x110:
		ev.mouse_button.button = RI_MOUSE_LEFT;
		break;
	case 0x111:
		ev.mouse_button.button = RI_MOUSE_RIGHT;
		break;
	case 0x112:
		ev.mouse_button.button = RI_MOUSE_MIDDLE;
		break;
	default:
		ev.mouse_button.button = RI_MOUSE_LEFT;
		break;
			   ...
			   struct wl_array *keys)
{
	...
	if (p->focused_window) {
		struct ri_event ev = { 0 };
		ev.type = RI_EVENT_WINDOW_FOCUS;
		ev.timestamp_ns = ri_wl_timestamp_ns();
		ri_wl_push_event(p->focused_window, &ev);
	}
}
			 ...
			 uint32_t state)
{
	...
	struct ri_window_impl *win = p->focused_window;
	struct ri_event ev = { 0 };
	ev.type = (state == WL_KEYBOARD_KEY_STATE_PRESSED) ? RI_EVENT_KEY_DOWN
							   : RI_EVENT_KEY_UP;
	ev.timestamp_ns = ri_wl_timestamp_ns();
	ev.key.scancode = key;
	...
	if (state == WL_KEYBOARD_KEY_STATE_PRESSED) {
		xkb_keysym_t sym =
			xkb_state_key_get_one_sym(p->xkb_state, key + 8);
...
void ri_wl_init_seat_listeners(struct ri_platform_impl *p)
{
	if (p->seat) {
...
static const char *cursor_name_from_enum(enum ri_cursor_shape shape)
{
	switch (shape) {
	case RI_CURSOR_DEFAULT:
		return "left_ptr";
	case RI_CURSOR_TEXT:
		return "xterm"; // ou "text"
	case RI_CURSOR_POINTER:
		return "hand1"; // ou "pointer"
	case RI_CURSOR_CROSSHAIR:
		return "crosshair";
	case RI_CURSOR_RESIZE_H:
		return "sb_h_double_arrow";
	case RI_CURSOR_RESIZE_V:
		return "sb_v_double_arrow";
	case RI_CURSOR_RESIZE_NWSE:
		return "fd_double_arrow";
	case RI_CURSOR_RESIZE_NESW:
		return "bd_double_arrow";
	case RI_CURSOR_GRAB:
		return "hand1";
	case RI_CURSOR_GRABBING:
		return "grabbing";
	default:
		return "left_ptr";
	}
...
void ri_window_set_cursor(ri_window_t window, enum ri_cursor_shape shape)
{
	if (!window)
		return;
	struct ri_platform_impl *p = window->platform;

	if (shape == RI_CURSOR_HIDDEN) {
		if (p->pointer) {
			wl_pointer_set_cursor(
				p->pointer, p->last_pointer_serial, NULL, 0, 0);
		}
		return;
	}
	...
	struct wl_cursor *cursor =
		wl_cursor_theme_get_cursor(p->cursor_theme, name);
	if (!cursor)
		return;
	...
	wl_pointer_set_cursor(p->pointer, p->last_pointer_serial,
			      p->cursor_surface, image->hotspot_x,
			      image->hotspot_y);
...
void ri_window_set_mouse_lock(ri_window_t window, bool locked)
{
	...
	struct ri_platform_impl *p = win->platform;

	if (locked && !win->mouse_locked) {
		/* Ativa lock */
		if (!p->pointer_constraints || !p->relative_pointer_manager) {
			RI_WL_LOG("aviso: pointer constraints não disponível "
				   "no compositor");
			return;
		}
		...
		zwp_relative_pointer_v1_add_listener(
			win->relative_pointer, &relative_pointer_listener, win);

		/* Esconde cursor */
		wl_pointer_set_cursor(p->pointer, p->last_pointer_serial, NULL,
				      0, 0);
		...
		/* Restaura cursor */
		ri_window_set_cursor(window, RI_CURSOR_DEFAULT);
