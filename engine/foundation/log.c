...
static struct {
	...
	.min_level = RI_LOG_LEVEL_INFO,
	.active_channels = RI_LOG_CHANNEL_ALL,
	.file_output = NULL,
	.colors_enabled = true,
	.initialized = false,
};
...
static const char *level_colors[] = {
	...
	RI_COLOR_CYAN,	   /* DEBUG */
	RI_COLOR_GREEN,   /* INFO */
	RI_COLOR_YELLOW,  /* WARN */
	RI_COLOR_RED,	   /* ERROR */
	RI_COLOR_MAGENTA, /* FATAL */
};
...
static const char *get_channel_name(ri_log_channel ch)
{
...
#ifdef NDEBUG
	g_log.min_level = RI_LOG_LEVEL_INFO;
#else
	g_log.min_level = RI_LOG_LEVEL_TRACE;
#endif
}
		    ...
		    const char *file, int line, const char *fmt, ...)
{
	va_list args;
	va_start(args, fmt);
	ri_log_output_v(level, channel, file, line, fmt, args);
	va_end(args);
}
		      ...
		      const char *file, int line, const char *fmt, va_list args)
{
	...
	/* Auto-init se esqueceram de chamar ri_log_init() */
	if (!g_log.initialized) {
		ri_log_init();
	}
	...
	if (g_log.colors_enabled) {
		fprintf(stderr, "%s%s %s[%-5s]%s %s[%-8s]%s %s[%s:%d]%s %s\n",
			RI_COLOR_GRAY, timestamp, level_colors[level], lvl_str,
			RI_COLOR_RESET, RI_COLOR_BLUE, ch_str,
			RI_COLOR_RESET, RI_COLOR_GRAY, filename, line,
			RI_COLOR_RESET, message);
	} else {
