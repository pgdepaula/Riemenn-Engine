			  ...
			  ri_gpu_buffer_t *buffer)
{
	...
	struct ri_gpu_buffer_impl *buf = calloc(1, sizeof(*buf));
	if (!buf)
	...
	if (config->usage & RI_BUFFER_VERTEX)
		usage |= VK_BUFFER_USAGE_VERTEX_BUFFER_BIT;
	if (config->usage & RI_BUFFER_INDEX)
		usage |= VK_BUFFER_USAGE_INDEX_BUFFER_BIT;
	if (config->usage & RI_BUFFER_UNIFORM)
		usage |= VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT;
	if (config->usage & RI_BUFFER_STORAGE)
		usage |= VK_BUFFER_USAGE_STORAGE_BUFFER_BIT;
	if (config->usage & RI_BUFFER_TRANSFER_SRC)
		usage |= VK_BUFFER_USAGE_TRANSFER_SRC_BIT;
	if (config->usage & RI_BUFFER_TRANSFER_DST)
	...
	VkResult result = vkCreateBuffer(device->device, &buffer_info, NULL,
					 &buf->buffer);
	...
	if (config->memory == RI_MEMORY_CPU_VISIBLE ||
	    config->memory == RI_MEMORY_CPU_TO_GPU) {
		mem_props = VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT |
			    VK_MEMORY_PROPERTY_HOST_COHERENT_BIT;
	}

	uint32_t mem_type = ri_vk_find_memory_type(
		device, mem_reqs.memoryTypeBits, mem_props);
