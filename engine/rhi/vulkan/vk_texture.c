			   ...
			   ri_gpu_texture_t *texture)
{
	...
	struct ri_gpu_texture_impl *tex = calloc(1, sizeof(*tex));
	if (!tex)
	...
	tex->format = ri_vk_format(config->format);
	tex->owns_image = true;

	VkImageUsageFlags usage = 0;
	if (config->usage & RI_TEXTURE_SAMPLED)
		usage |= VK_IMAGE_USAGE_SAMPLED_BIT;
	if (config->usage & RI_TEXTURE_STORAGE)
		usage |= VK_IMAGE_USAGE_STORAGE_BIT;
	if (config->usage & RI_TEXTURE_RENDER_TARGET)
		usage |= VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT;
	if (config->usage & RI_TEXTURE_DEPTH_STENCIL)
	...
	VkImageCreateInfo image_info = {
		...
		.arrayLayers =
			config->array_layers > 0 ? config->array_layers : 1,
	...
	uint32_t mem_type =
		ri_vk_find_memory_type(device, mem_reqs.memoryTypeBits,
	...
	result =
		vkCreateImageView(device->device, &view_info, NULL, &tex->view);
	if (result != VK_SUCCESS) {
...
void ri_gpu_texture_destroy(ri_gpu_texture_t texture)
{
	...
	if (texture->view)
		vkDestroyImageView(texture->device->device, texture->view,
				   NULL);

	if (texture->owns_image) {
		if (texture->image)
			vkDestroyImage(texture->device->device, texture->image,
				       NULL);
		if (texture->memory)
			vkFreeMemory(texture->device->device, texture->memory,
				     NULL);
	}
...
static void ri_vk_transition_layout(VkCommandBuffer cmd, VkImage image,
				     ...
				     VkImageLayout new_layout)
{
	...
	vkCmdPipelineBarrier(cmd, source_stage, dest_stage, 0, 0, NULL, 0, NULL,
			     1, &barrier);
}
			   ...
			   uint32_t array_layer, const void *data, size_t size)
{
	...
	struct ri_gpu_device_impl *dev = texture->device;
	VkDevice device = dev->device;

	/* 1. Criar Staging Buffer */
	ri_gpu_buffer_t staging_buf;
	struct ri_gpu_buffer_config buf_config = {
		.size = size,
		.usage = RI_BUFFER_TRANSFER_SRC,
		.memory = RI_MEMORY_CPU_TO_GPU,
		.label = "Staging Texture Upload",
	};

	if (ri_gpu_buffer_create(dev, &buf_config, &staging_buf) !=
	    RI_GPU_OK) {
		return RI_GPU_ERR_NOMEM;
	}

	/* 2. Copiar dados para o buffer */
	if (ri_gpu_buffer_upload(staging_buf, 0, data, size) != RI_GPU_OK) {
		ri_gpu_buffer_destroy(staging_buf);
		return RI_GPU_ERR_DEVICE;
	}
	...
	ri_vk_transition_layout(cmd, texture->image, texture->format,
			   ...
			   ri_gpu_sampler_t *sampler)
{
	...
	struct ri_gpu_sampler_impl *s = calloc(1, sizeof(*s));
	if (!s)
		return RI_GPU_ERR_NOMEM;
	...
	VkSamplerCreateInfo sampler_info = {
		...
		.mipmapMode = config->mip_filter == RI_FILTER_LINEAR
	...
	VkResult result = vkCreateSampler(device->device, &sampler_info, NULL,
					  &s->sampler);
	if (result != VK_SUCCESS) {
		free(s);
		return RI_GPU_ERR_DEVICE;
