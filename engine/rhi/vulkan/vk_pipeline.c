...
static VkPrimitiveTopology vk_primitive_from_bhs(enum ri_gpu_primitive p)
{
	switch (p) {
	case RI_PRIMITIVE_TRIANGLES:
		return VK_PRIMITIVE_TOPOLOGY_TRIANGLE_LIST;
	case RI_PRIMITIVE_TRIANGLE_STRIP:
		return VK_PRIMITIVE_TOPOLOGY_TRIANGLE_STRIP;
	case RI_PRIMITIVE_LINES:
		return VK_PRIMITIVE_TOPOLOGY_LINE_LIST;
	case RI_PRIMITIVE_LINE_STRIP:
		return VK_PRIMITIVE_TOPOLOGY_LINE_STRIP;
	case RI_PRIMITIVE_POINTS:
...
static VkCullModeFlags vk_cull_mode_from_bhs(enum ri_gpu_cull_mode c)
{
	switch (c) {
	case RI_CULL_NONE:
...
static VkBlendFactor vk_blend_factor_from_bhs(enum ri_gpu_blend_factor f)
{
	switch (f) {
	case RI_BLEND_ZERO:
...
static VkBlendOp vk_blend_op_from_bhs(enum ri_gpu_blend_op o)
{
	switch (o) {
...
static VkCompareOp vk_compare_op_from_bhs(enum ri_gpu_compare_func f)
{
	switch (f) {
			  ...
			  ri_gpu_shader_t *shader)
{
	...
	struct ri_gpu_shader_impl *s = calloc(1, sizeof(*s));
	if (!s)
			    ...
			    ri_gpu_pipeline_t *pipeline)
{
	...
	struct ri_gpu_pipeline_impl *p = calloc(1, sizeof(*p));
	if (!p)
		return RI_GPU_ERR_NOMEM;
	...
	for (uint32_t i = 0; i < config->color_format_count && i < 8; i++) {
		attachments[att_count] = (VkAttachmentDescription){
			.format = ri_vk_format(config->color_formats[i]),
	...
	if (config->depth_test || config->depth_write) {
		attachments[att_count] = (VkAttachmentDescription){
			.format = ri_vk_format(config->depth_format),
			.samples = VK_SAMPLE_COUNT_1_BIT,
	...
	for (uint32_t i = 0; i < config->vertex_attr_count && i < 16; i++) {
		...
		attrs[i].format = ri_vk_format(config->vertex_attrs[i].format);
	...
	if (config->blend_state_count > 0) {
		const struct ri_gpu_blend_state *bs = &config->blend_states[0];
	...
	ri_gpu_pipeline_t *pipeline)
{
	...
	struct ri_gpu_device_impl *dev = device;
	struct ri_gpu_pipeline_impl *pipe = calloc(1, sizeof(*pipe));
	if (!pipe)
		return RI_GPU_ERR_NOMEM;
   ...
   * ri_gpu_cmd_push_constants */
	...
	/* Shader Stage */
	struct ri_gpu_shader_impl *shader = config->compute_shader;
	if (!shader) {
			      ...
			      ri_gpu_cmd_buffer_t *cmd)
{
	...
	struct ri_gpu_cmd_buffer_impl *c = calloc(1, sizeof(*c));
	if (!c)
		return RI_GPU_ERR_NOMEM;
...
void ri_gpu_cmd_set_index_buffer(ri_gpu_cmd_buffer_t cmd,
				  ri_gpu_buffer_t buffer, uint64_t offset,
				  bool is_32bit)
{
	if (!cmd || !buffer)
			      ...
			      ri_gpu_sampler_t sampler)
{
	...
	if (cmd->current_pipeline_layout == VK_NULL_HANDLE) {
		RI_VK_LOG("aviso: bind_texture chamado sem pipeline setado");
		return;
	}

	struct ri_gpu_device_impl *dev = cmd->device;
	...
	/* Atualiza set */
	struct ri_gpu_texture_impl *tex = texture;
	struct ri_gpu_sampler_impl *sam = sampler;
				...
				const void *data, uint32_t size)
{
	...
	/* Usamos a união de todos os estágios suportados pelo gui para evitar
				   ...
				   const struct ri_gpu_render_pass *pass)
{
	...
	/* HACK: Criando RenderPass e Framebuffer on-the-fly é terrível para
     ...
     Idealmente: `ri_gpu_render_pass` deveria ser um objeto opaco criado antes,
  ...
  */
	...
	struct ri_gpu_swapchain_impl *sc = cmd->device->swapchain;
	if (sc) {
...
/**
 * ri_gpu_cmd_bind_compute_storage_texture - Binda storage image para compute
 */
					      ...
					      ri_gpu_texture_t texture)
{
	...
	if (vkAllocateDescriptorSets(dev->device, &alloc_info, &desc_set) !=
	    VK_SUCCESS) {
		RI_VK_LOG("aviso: falha aloc set compute");
		return;
	}
