				 ...
				 VkMemoryPropertyFlags properties)
{
	for (uint32_t i = 0; i < dev->memory_properties.memoryTypeCount; i++) {
		if ((type_filter & (1 << i)) &&
		    (dev->memory_properties.memoryTypes[i].propertyFlags &
...
VkFormat ri_vk_format(enum ri_gpu_texture_format fmt)
{
	...
	case RI_FORMAT_R32_FLOAT:
		return VK_FORMAT_R32_SFLOAT;
	case RI_FORMAT_RG32_FLOAT:
		return VK_FORMAT_R32G32_SFLOAT;
	case RI_FORMAT_RGB32_FLOAT:
		return VK_FORMAT_R32G32B32_SFLOAT;
	case RI_FORMAT_RGBA32_FLOAT:
		return VK_FORMAT_R32G32B32A32_SFLOAT;
	case RI_FORMAT_DEPTH32_FLOAT:
		return VK_FORMAT_D32_SFLOAT;
	case RI_FORMAT_DEPTH24_STENCIL8:
			  ...
			  ri_gpu_device_t *device)
{
	...
	struct ri_gpu_device_impl *dev = calloc(1, sizeof(*dev));
	if (!dev)
	...
	for (uint32_t i = 0; i < required_count; i++) {
		...
		for (uint32_t j = 0; j < ext_count_available; j++) {
			if (strcmp(required_extensions[i],
				   available_exts[j].extensionName) == 0) {
				found = true;
		...
		if (!found) {
			RI_VK_LOG("aviso: extensão %s não suportada pelo "
				   "driver Vulkan",
				   required_extensions[i]);
		}
	}
	...
	vkEnumerateInstanceLayerProperties(&layer_count_available,
					   available_layers);
	...
	if (config->enable_validation) {
		...
		for (uint32_t i = 0; i < layer_count_available; i++) {
			if (strcmp(validation, available_layers[i].layerName) ==
			    0) {
				found = true;
		...
		if (!found) {
			RI_VK_LOG("aviso: validation layer solicitada mas não "
				   "encontrada");
		}
	}
	...
	RI_VK_CHECK(result, "falha ao criar VkInstance");
	...
	if (gpu_count == 0) {
		RI_VK_LOG("erro: nenhuma GPU com Vulkan encontrada");
	...
	RI_VK_LOG("usando GPU: %s", dev->properties.deviceName);
	...
	if (dev->graphics_family == UINT32_MAX) {
		RI_VK_LOG("erro: GPU não tem queue de gráficos");
		vkDestroyInstance(dev->instance, NULL);
		free(dev);
		return RI_GPU_ERR_DEVICE;
	}
	...
	result = vkCreateDevice(dev->physical_device, &device_create_info, NULL,
				&dev->device);
	RI_VK_CHECK(result, "falha ao criar VkDevice");

	vkGetDeviceQueue(dev->device, dev->graphics_family, 0,
			 &dev->graphics_queue);
	...
	result = vkCreateCommandPool(dev->device, &pool_info, NULL,
				     &dev->command_pool);
	RI_VK_CHECK(result, "falha ao criar command pool");
...
void ri_gpu_device_destroy(ri_gpu_device_t device)
{
	...
	if (device->texture_layout)
		vkDestroyDescriptorSetLayout(device->device,
					     device->texture_layout, NULL);

	if (device->command_pool)
		vkDestroyCommandPool(device->device, device->command_pool,
				     NULL);
