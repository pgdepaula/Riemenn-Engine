 ...
 *
 * Uso:
 *   RI_TEST_BEGIN("Minha Suite");
 *   RI_TEST_ASSERT(1 + 1 == 2, "Matemática quebrou");
 *   RI_TEST_ASSERT(ptr != NULL, "Ponteiro nulo");
 *   RI_TEST_END();
 */

#ifndef RI_TEST_RUNNER_H
#define RI_TEST_RUNNER_H
...
/**
 * RI_TEST_BEGIN - Inicia uma suite de testes
 */
#define RI_TEST_BEGIN(suite_name)                                             \
	do {                                                                   \
...
/**
 * RI_TEST_ASSERT - Verifica uma condição
 */
#define RI_TEST_ASSERT(cond, msg)                                             \
	do {                                                                   \
		if (cond) {                                                    \
			g_test_passed++;                                       \
...
/**
 * RI_TEST_ASSERT_EQ - Verifica igualdade de inteiros
 */
#define RI_TEST_ASSERT_EQ(a, b, msg)                                          \
	do {                                                                   \
...
/**
 * RI_TEST_ASSERT_NOT_NULL - Verifica ponteiro não-nulo
 */
#define RI_TEST_ASSERT_NOT_NULL(ptr, msg)                                     \
	do {                                                                   \
		if ((ptr) != NULL) {                                           \
			g_test_passed++;                                       \
			printf("  %s[PASS]%s %s\n", TEST_GREEN, TEST_RESET,    \
...
/**
 * RI_TEST_END - Finaliza a suite e retorna código de saída
 */
#define RI_TEST_END()                                                         \
	do {                                                                   \
		printf("\n%s----------------------------------------%s\n",     \
...
/**
 * RI_TEST_SECTION - Marca uma seção dentro da suite
 */
#define RI_TEST_SECTION(name)                                                 \
	printf("\n  %s>> %s%s\n", TEST_YELLOW, name, TEST_RESET)

#endif /* RI_TEST_RUNNER_H */
